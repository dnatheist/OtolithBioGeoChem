...
#Including DC13, D15N and Mean Otolith Length
## Analysis Based on raw Variables

```{r, "Cluster Analysis Based on raw Variables with ID and mean OL"}
...
#and with larvaID
plot(hClusters,labels=(df$row.names), hang = -1)

# colors the leaves of a dendrogram
y = cutree(hClusters, 12)
ColorDendrogram(hClusters, y = y, labels=df$Site.Name, main = "Nests", branchlength = 3)
...
df <- subset(df, select = c(meanOL,Delta13C,Delta15N))
...
#K means test run
kmeansObj <- kmeans(df,centers=10)
names(kmeansObj)

par(mar=rep(0.2,4))
plot(df,col=kmeansObj$cluster,pch=19,cex=2)
plot(kmeansObj$centers,col=1:10,pch=3,cex=3,lwd=3)
```
In this case there are `r nrow(df)` rows of larvae  `r row.names(df)`  and `r ncol(df)` variables `r names(df)` in the dataframe.
...
#This Time the Analysis Includes a estimate of Nest Location based on dispersal days and capture location

This analysis includes an 'arbitrary' nest location calculation based on age of the larva as determined from otolith length, the time it is likely to have had available for dispersal (age - 7), and distance above the capture site calculated at 300 metres movement per day. The formula in r is as follows:
...
##Cluster Analysis Based on raw Variables 

```{r echo=FALSE, "Base Work including a nest site estimate"}
...
set.seed(143)
##Remove non-numerics for matrix construction as Heatmap requires
df <- subset(df, select = c(nest,Delta13C,Delta15N))
dataMatrix<-as.matrix(df)
heatmap(dataMatrix)
